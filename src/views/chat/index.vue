...
function contextualAssemblyData() {
  ...
  if (!usingContext.value) {
    ...
    if (lastChat) {
      const my = { role: 'user', content: lastChat.text }
  ...
  const sys = { role: 'system', content: settingStore.systemMessage }
  conversation.push(sys)

  for (const chat of dataSources.value) {
    if (chat.inversion && !chat.conversationOptions) {
      const my = { role: 'user', content: chat.text }
      conversation.push(my)
    }
    else if (chat.text !== '') {
      const ai = { role: 'assistant', content: chat.text }
